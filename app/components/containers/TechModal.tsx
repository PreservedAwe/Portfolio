...
export default function Modal() {
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ ease: "easeIn", duration: 0.3 }} className="bg-black bg-opacity-25 backdrop-blur-sm fixed top-0 left-0 w-screen h-screen flex justify-center items-center z-20">
            <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ ease: "easeIn", duration: 1.0 }} className="bg-black size-6/12 rounded-md flex flex-col justify-start items-center p-4 overflow-y-auto overflow-x-hidden border-white border-4 gap-2">
                ...
                <div className="flex flex-wrap gap-5 overflow-y-auto overflow-x-hidden justify-center items-center bg-white rounded-lg mx-8 my-2 p-4 size-10/12 border-4 border-blue-500">
                    ...
                    <motion.img animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 7, ease: "linear" }} className="size-20" src="/threejs.svg" alt="threejs" />
                </div>
            </motion.div>
        </motion.div>
    );
